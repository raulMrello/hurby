/*************************************************************************************
		This file is the implementation file of the Satiation class
 *************************************************************************************/
...
static float buf_in[]={ 0,TSLP_RANGE/2,TSLP_RANGE,
						0,PTT_RANGE/2, PTT_RANGE};  // center points of each fuzzy set: In_0], In_1
...
/** Constructor **********************************************************************/
Satiation::Satiation(uint8_t prio){
	this->serial = 0;
	...
	this->bhv.level = prio;
	this->bhv.obj = this;
	this->bhv.SetInput = Satiation::SetInput;
	this->bhv.SetInputParam = Satiation::SetInputParam;
	this->bhv.RunBehaviour = Satiation::RunBehaviour;
	this->bhv.GetOutput = Satiation::GetOutput;	
	strcpy(this->action.layerName, "satiation");
	// setup fuzzy inference instance
	this->fuzzy = new Fuzzy(2, buf_in, 1, buf_ou, rules);
}

/** Destructor ***********************************************************************/
Satiation::~Satiation(void){
	if(this->fuzzy){
		delete(this->fuzzy);
	}
}
...
/** Gets  behaviour ******************************************************************/
BEHAVIOUR_T* Satiation::GetBehaviour(void){ 
	return(&this->bhv);
}

/** En/Dis logger ********************************************************************/
void Satiation::EnDisDebugging(Serial * serial, int8_t level){
	if(level <= 0){
		this->serial = 0;
		return;
	}
...
/** Updates its inputs **************************************************************/
void Satiation::Run(float modulation){
	if(this->serial){
		this->serial->printf("S74\r\n");
	}
	float formattedInput[2];
	formattedInput[0] = (float)this->tslp;
	formattedInput[1] = (float)this->ptt;
	this->lastActivity = this->activity;
	this->fuzzy->Execute(formattedInput, &this->satiation, 0, 0);
	this->activity = 100 - this->satiation;
 ...
 */
void Satiation::SetAction(float modulation){
	if(this->serial){
		this->serial->printf("S109\r\n");
	}
	...
	this->action.mouth_sound_count = 0; // Can reproduce up to 3 diferent voice/sound recordings (example of use)
	...
	if(modulation <= (this->lastModulation - 10)){
		this->lastModulation = modulation;
		this->action.mouth_sound_count = 2; // Can reproduce up to 3 diferent voice/sound recordings (example of use)
		for(uint8_t i=0; i<this->action.mouth_sound_count;i++)
			this->action.mouth_sound_list[i] = i+12;
	...
	//else if happier say "Yuppie" instantaneously
		...
		this->action.mouth_sound_count = 2; // Can reproduce up to 3 diferent voice/sound recordings (example of use)
		for(uint8_t i=0; i<this->action.mouth_sound_count;i++)
			this->action.mouth_sound_list[i] = i+14;
		// if repeat range is [x,0] it will talked delayed but only once
		// if repeat range is [x,y] it will talked delayed betwwen x and y multiple times
		// if repeat range is [0,0] it will talked instantaneouly only once
		// if repeat range is [0,y] it will talked instantaneouly and repeated each y delay
	...
	else if(this->satiation == 100){
		if(rnd(0.0, 1.0) < 0.7){
			this->action.mouth_sound_count = 4; // Can reproduce up to 3 diferent voice/sound recordings (example of use)
			for(uint8_t i=0; i<this->action.mouth_sound_count;i++)
				this->action.mouth_sound_list[i] = i+12;
			// if repeat range is [x,0] it will talked delayed but only once
			// if repeat range is [x,y] it will talked delayed betwwen x and y multiple times
